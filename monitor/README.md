...
| 외부 접속 인증 | `src/proxy.ts` (Next.js Proxy, Node.js 런타임) + `server.js` + Redis — 아래 "외부 접속 인증" 참고 |

## 디렉토리 구조

```
monitor/
├── server.js                  # 커스텀 서버 — 신뢰 가능한 피어에서 온 요청만 x-forwarded-for/x-real-ip를 통과시킴
├── src/
...
│       ├── ip.ts               # 내부 IP 판별, x-forwarded-for 파싱, IPv6 /64 rate-limit 버킷팅
...
외부 IP 접속
  ...
  → 세션 쿠키가 유효한가? → 통과
  → /auth로 리다이렉트
       → 페이지 로드 시 자동으로 POST /api/auth/request-code (AuthForm.tsx 마운트 시점)
       ...
       → 6자리 코드 입력(자동 제출) 또는 "Discord DM 재전송"으로 재요청 → POST /api/auth/verify-code
...
- **`x-forwarded-for` 신뢰 모델**: `next start` 대신 `server.js`(커스텀 서버)로
  구동한다 — Route Handler·`proxy.ts`는 원본 TCP 소켓 주소를 볼 수 없으므로,
  `server.js`가 Next에 요청을 넘기기 전에 `req.socket.remoteAddress`를 직접 확인해
  루프백이나 `MONITOR_TRUSTED_PROXY_CIDRS`에 등록된 주소가 아니면
  `x-forwarded-for`/`x-real-ip`를 실제 접속 IP로 강제 치환한다. 그래서 리버스
  프록시 없이 인터넷에 직접 노출해도 클라이언트가 이 헤더를 위조해 "내부 IP"로
  위장할 수 없다. 리버스 프록시를 별도 장비에 둔다면 그 장비 주소를
  `MONITOR_TRUSTED_PROXY_CIDRS`에 등록해야 프록시가 설정한 값이 통과된다.
- **쿠키는 HTTPS 전제(`Secure`)로 설정된다.** 리버스 프록시가 TLS를 종단하지
  ...
  개발에는 영향이 없다.
- **IPv6 시도 횟수·차단은 /64 단위로 묶인다** — ISP가 보통 가정용 회선 하나에
  /64 이상을 통째로 위임하므로, 주소 하나씩 카운트하면 같은 /64 안에서 주소를
  바꿔가며 영구 차단을 무력화할 수 있다 (`src/lib/ip.ts`의 `rateLimitBucket()`).
