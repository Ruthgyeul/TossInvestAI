...
from core.gateway.base import load_reflection_prompt
from core.gateway.claude import call_freeform
...
async def _call_claude_reflection(market: Market, trades: list[dict], rejections: list[dict]) -> str:
    ...
    core/gateway/claude.py `call_freeform`을 통해서만 호출한다 — 이 모듈이 별도
    Anthropic 클라이언트를 두지 않는다 (docs/CODING_RULES.md "모든 AI 호출은
    core/gateway/ 모듈에서만 수행한다").
    """
    user_message = (
        f"[{market} 시장 오늘 체결 내역]\n{_summarize_trades(trades)}\n\n"
        f"[{market} 시장 오늘 Safety Gate 거부 내역]\n{_summarize_rejections(rejections)}"
    )
    return await call_freeform(load_reflection_prompt(), user_message)
