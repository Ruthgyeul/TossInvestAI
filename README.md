...
| 키오스크 모니터 | Next.js + TypeScript | 7인치 모니터 24/7 상태판, 상호작용 없음 (`monitor/`) |
...
monitor/                # Next.js 키오스크 모니터 — 7인치 모니터 24/7 상태판 (docs/MONITOR.md 참고)
...
| [`docs/MONITOR.md`](./docs/MONITOR.md) | 키오스크 모니터(`monitor/`) 설계 근거, 데이터 흐름, 화면 구성 |
...
# 키오스크 모니터 (7인치 모니터에 Chromium --kiosk로 http://localhost:3000 표시)
cd monitor && npm run build && npm start   # 또는 개발 중엔 npm run dev
```
운영 환경 배포·systemd 등록은 [`docs/DEPLOYMENT.md`](./docs/DEPLOYMENT.md) 참고,
모니터 상세는 [`monitor/README.md`](./monitor/README.md)·[`docs/MONITOR.md`](./docs/MONITOR.md) 참고.
...
# 키오스크 모니터 타입 체크·린트·빌드
cd monitor && npm run lint && npm run build
```
