...
| `GET /api/v1/health` | `/health` | `health.ts` | — | `HealthSnapshot`(`core/monitoring/health.py`: `cpu_pct, memory_pct, disk_pct, temp_c, toss_api_reachable`) |
...
```json
{
  "event_type": "trade_executed | safety_rejection | emergency_stop | health_alert | report_ready | backtest_complete | status_update | reflection_ready | news_summary | version_candidate_ready",
